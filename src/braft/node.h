...
private:
    ...
    SnapshotExecutor* _snapshot_executor;//leader用来执行snapshot操作
    ReplicatorGroup _replicator_group;
    std::vector<Closure*> _shutdown_continuations;
    ElectionTimer _election_timer;//随机的选举超时时间，leader在这个随机时间没有和follow通信就会触发follow进行选举
...
附加日志 RPC：由领导人负责调用来复制日志指令；也会用作heartbeat
    term            领导人的任期号
    leaderId        领导人的 Id，以便于跟随者重定向请求
    prevLogIndex    新的日志条目紧随之前的索引值
    prevLogTerm     prevLogIndex 条目对应的任期号
...
接收者实现：
    ...
    4. 附加日志中尚未存在的任何新条目
    5. 如果 leaderCommit > commitIndex ，令 commitIndex 等于 leaderCommit 和 新日志
        条目索引值中较小的一个

请求投票 RPC：候选人负责调用用来征集选票
    term:           候选人的任期号
...
所有服务器需遵守的规则:
    ...
    2.如果接收到的 RPC 请求或响应中，任期号 T > currentTerm ，那么就令currentTerm 等于 T，并切换状态为跟随者
...
领导人:
    ...
    3. 如果对于一个跟随者，最后日志条目的索引值大于等于 nextIndex，那么：发送从nextIndex 开始的所有日志条目
        a.如果成功：更新相应跟随者的 nextIndex 和 matchIndex
